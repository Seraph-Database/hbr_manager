<template>
  <v-hover>
    <template v-slot:default="{ isHovering, props }">
      <v-scale-transition appear>
        <v-btn
          v-if="!dataStore.loading && dataStore.getStyles"
        ...
        >
          ...
          <div class="text-HBR text-white mb-1">{{ `Edit all` }}</div>
        </v-btn>
      </v-scale-transition>
    </template>
  </v-hover>
  ...
  >
    ...
    <v-card
      ...
      v-click-outside="cancelToggleAll"
    >
      ...
      >
        ...
        >
          <v-list-item
            class="px-2"
            v-for="style in dataStore.getStyles?.filter(searchFilter)"
            :key="style.id"
            :title="`${style.name}`"
          >
            <template v-slot:prepend>
      ...
      <div
        v-if="dataStore.getStyles"
        class="text-normal text-center text-HBR mx-3 mb-3"
      >
        {{
          dataStore.isAllVisibleOwned(
            dataStore.getStyles
              .filter(searchFilter)
              .map((s) => [s.id, s.max_lb])
          )
...
const confirmToggleAll = () => {
  ...
  if (dataStore.getStyles) {
    dataStore.toggleAllVisibleMax(
      dataStore.getStyles.filter(searchFilter).map((s) => [s.id, s.max_lb])
    );
  }
};
</script>
